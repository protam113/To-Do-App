...
class CustomRuntimePlugin {
    ...
    apply(compiler) {
        compiler.hooks.make.tapAsync('CustomRuntimePlugin', (compilation, callback) => {
            ...
            sdk_1.infrastructureLogger.log('Creating child compiler for', this.bundlerRuntimePath);
            childCompiler.hooks.thisCompilation.tap(this.constructor.name, (childCompilation) => {
                childCompilation.hooks.processAssets.tap(this.constructor.name, () => {
                    ...
                    if (entryChunk) {
                        ...
                        const entryModuleId = childCompilation.chunkGraph.getModuleId(entryModule);
                        this.entryModule =
                    ...
                    sdk_1.infrastructureLogger.log('got compilation asset');
            ...
            childCompiler.runAsChild((err, entries, childCompilation) => {
                ...
                if (!childCompilation) {
                    sdk_1.infrastructureLogger.warn('Embed Federation Runtime: Child compilation is undefined');
                    return callback();
                }
                if (childCompilation.errors.length) {
                    return callback(childCompilation.errors[0]);
                }
                sdk_1.infrastructureLogger.log('Code built successfully');
                callback();
            });
        });
        compiler.hooks.thisCompilation.tap('CustomRuntimePlugin', (compilation) => {
            const handler = (chunk, runtimeRequirements) => {
                ...
                sdk_1.infrastructureLogger.log(`Custom runtime module added to chunk: ${chunk.name}`);
            };
