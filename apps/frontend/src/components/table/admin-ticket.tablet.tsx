...
export const AdminTicketTable: React.FC<AdminTickTableProps> = ({
  tickets,
  isLoading,
  isError,
  refreshKey,
  ...
  const { mutate: deleteTicket } = useDeleteTicket(deletingId || '');

  const handleDelete = (ticketId: string) => {
    if (
      window.confirm(`Are you sure you want to delete ticket "${ticketId}"?`)
    ) {
              ...
              tickets.map((ticket) => (
                <TableRow key={ticket.id}>
                  {AdminTicketColumns.map((col) => {
                    ...
                    if (col.key === 'title') {
                      cellContent = <p> {truncateText(ticket.title, 20)}</p>;
                      ...
                      cellContent = (
                        <div className="flex items-center gap-1">
                          <Button
                            size="sm"
                            variant="ghost"
                            className="flex items-center gap-1 text-sm text-blue-600 hover:bg-blue-50"
                            onClick={() =>
