...
//UI components
import { useDeleteReportId } from '@/hooks';
...
export const AdminReportListTable: React.FC<ReportTableProps> = ({
  reports,
  isLoading,
  isError,
  refreshKey,
  ...
  const { mutate: deleteReport } = useDeleteReportId();

  const handleDeleteReport = (reportId: string) => {
    if (window.confirm('Are you sure you want to delete this report?')) {
      deleteReport(reportId, {
        onSuccess: () => {
          // Trigger refresh sau khi xóa thành công
          if (refreshKey) {
            refreshKey();
          }
        },
                ...
                <TableRow key={report.id}>
                  {TeamTaskColumns.map((col) => {
                    ...
                    if (col.key === 'title') {
                      ...
                      cellContent = (
                        <div className="flex items-center gap-3">
                          <span className="font-medium">{updatedBy.name}</span>
                        </div>
                      );
                      ...
                      cellContent = (
                        <div className="flex items-center gap-3">
                          <span className="font-medium">{createdBy.time}</span>
                        </div>
                      );
                      ...
                      cellContent = (
                        <div className="flex items-center gap-3">
                          <span className="font-medium">{updatedBy.time}</span>
                        </div>
                      );
