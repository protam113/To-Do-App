...
import { AdminProjectTable } from '@/components/table/admin-project.table';
import { AdminProjectList } from '@/libs/responses/projectLib';
import { TeamSelect } from '@/components/select/team-select';
...
export default function Page() {
  ...
  const params = {
    ...
    page_size: pageSize,
    teamId: selectTeamId || undefined,
  };

  const { projects, isLoading, isError, pagination } = AdminProjectList(
    currentPage,
    params,
    refreshKey
  );
      ...
      <Container>
        ...
        <div className="md:flex col flex-col-2 md:flex-row justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              <span className="text-xs text-main font-semibold">Team :</span>
              <TeamSelect
                value={selectTeamId || 'all'}
                onValueChange={(value) => {
                  setSelectTeamId(value === 'all' ? '' : value);
                  setCurrentPage(1);
                }}
