...
import { TypeTable } from '@/components/table/type.table';
import { TypeList } from '@/libs/responses/typeLib';
import CreateType from '@/components/pages/type/type.create';
...
export default function Page() {
  ...
  const { types, isLoading, isError, pagination } = TypeList(
    currentPage,
    params,
    refreshKey
  );
  ...
  return (
    <>
      <Container className="min-h-screen p-6  ">
        <Header title="Type Management Page" />
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          ...
          <Button
            ...
            className={` text-xs border-b-2 text-main border-b-main ${
              isCreateDialogOpen
                ? ' bg-main-100 '
                : ' hover:bg-main-900 hover:text-white'
            }`}
          >
            {isCreateDialogOpen ? 'Cancel' : 'Create Type'}
          </Button>
        </div>

        {isCreateDialogOpen && (
          <div className="border-2 bg-white border-main   p-6 shadow-lg animate-in fade-in slide-in-from-top-4 duration-300">
            <CreateType onSuccess={() => setIsCreateDialogOpen(false)} />
          </div>
        )}
        {!isCreateDialogOpen && (
          <>
